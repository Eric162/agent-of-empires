...
export interface SessionResponse {
   ...
   *  the user hasn't viewed) or `"manual"` (a deliberate flag), null /
   *  undefined when read. The sidebar paints an unread accent and offers a
   *  right-click "Mark as read/unread" toggle, both gated on the
   *  `session.unread_indicator` setting. The chip is suppressed for the
   *  session currently open, which also clears an `"auto"` marker. */
